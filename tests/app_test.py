...
def test_get_individual_request():
    ...
    response = requests.get(
        f'{BASE_URL}/request/04cfc704-acb2-40af-a8d3-4611fab54ada'
    )
...
def test_get_individual_request_404():
    ...
    response = requests.get(f'{BASE_URL}/request/an_incorrect_id')
...
def test_get_new_record():
    ...
    url = f'{BASE_URL}/request/{NewUUID.value}'
...
def test_edit_new_record_title():
    ...
    response = requests.put(f'{BASE_URL}/request/{NewUUID.value}', json=payload)
...
def test_delete_new_record():
    ...
    response = requests.delete(f'{BASE_URL}/request/{NewUUID.value}')
